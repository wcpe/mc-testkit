...
### 新增
- **场景生命周期钩子（ADR-0020）**：`scenario` 新增 `beforeScenario` / `readyScenario` / `afterScenario` 三个时序点的钩子，供消费方插入时序敏感动作，而不必自建整套编排（ADR-0017 的重路线）。**为什么要三个点而不是一个**：被测系统依赖外部控制面时，天然有「起进程 → 服务端注册 → 审批」三段时序——压缩成一段会导致要么审批在服务端起来前做（失败），要么控制面在服务端起来后才起（agent 注册失败且拖慢）。三点分别落在：运行目录就绪后 / 服务端启动前（起外部依赖）、全部节点端口就绪后 / bot 启动前（依赖服务端的初始化）、场景判定之后（收尾）。配套五个内置实现：`ExecHook`（起后台进程，支持 TCP / 日志双就绪门）、`StopPidHook`（按 pid 收尾，进程已退出静默跳过）、`HttpHook`（调 admin API，用 `HttpURLConnection` 以不抬高最低运行 JDK）、`SleepHook`、`HookChain`（多步串联，任一步失败即中断）。**失败路径保障**：收尾钩子挂 `finalizedBy` 并与既有 `stop*` 同层，正常 / 失败 / 中断三路径都执行；且收尾钩子抛异常只记 warn 不阻断其余清理、不掩盖场景判定失败——与 ADR-0004「收尾不残留」红线一致。前置钩子则失败即抛出（初始化没成功，后续跑下去只会得到误导性结果）。`readyScenario` 仅在集群场景提供（直连 / 经代理的后端是前台自停进程，不存在「全部节点已就绪」的时刻），其他形态声明会在配置期报中文错误——不静默忽略。钩子实现为具名类而非 lambda：配置缓存要求动作捕获图可序列化，而 Kotlin 的 SAM 转换产物不是 `Serializable`。

### 修复
- **残留进程收尾不掉、且起服失败时归因误导（FR-04 / FR-17 / FR-18，NFR「收尾干净」）**：`stop<Key>Serve` 等停任务此前只按 pid 文件收尾，而 **pid 文件是「当前一轮」的凭证**——`ServerLauncher.launch` 每次起服都会先删旧 pid 再写新的，运行目录清理还会整片删掉它（保留清单只有 `libraries/cache/assets/versions`）。于是上一轮构建被强杀（关终端 / 杀 daemon，shutdown hook 与 `finally` 都没跑到）留下的进程，其 pid 记录已随下一轮消失：`stopProcessByPidFile` 遇到「pid 文件不存在」一律按「本就没起来」静默 no-op，**残留进程永远收尾不掉**，且持续占着端口让后续每一轮起服都失败。更糟的是同一场景下 `awaitPortOpen` 以「端口可连」为就绪信号，残留进程同样让端口可连，于是新进程被**误判为已就绪**（日志先打「✅ 已就绪」，随后新服务端真正 bind 时以 `bind(..) failed: 地址已在使用` 崩溃）——用户看到「起服成功又立刻失败」，归因成本极高。本次三处一起修：
  - **起服前端口占用预检**（`task/PortGuard.kt`）：单后端 / 经代理 / 集群 / 压测 / serve（单 + 集群）全部起服路径在起服前检查目标端口，被占即**中文失败**，报错含被占端口、用途标签、按端口排查的命令（Windows / Linux 各给对应命令）与收尾任务建议；不再出现上述误导形态。判定用「可绑定 **且** 不可连接」双信号：单看试绑会在 Windows 上误判（`SO_REUSEADDR` 语义与 Unix 相反，允许两个活着的 socket 绑同一端口），单看连接则会把 `TIME_WAIT` 状态误报为占用。
  - **进程台账**（`task/ProcessLedger.kt`）：起服时除 pid 文件外，另把进程登记到 `<buildDir>/mc-testkit/results/process-ledger.properties`——该目录**无任何清理点**（随 `build/` 走），故记录能跨轮次存活。登记时**就地抓取**该进程的命令行与启动时刻（而非让调用方传 jar 路径猜）：启动命令行随形态而变（自包含 jar 含 jar 路径，thin jar 只含 classpath 启动器路径），就地抓取才对得上。停任务先按 pid 文件（快路径）、再按台账兜底（跨轮次路径），收尾后自动注销记录。**安全边界**：台账路径在杀之前核对「命令行相同 **且** 启动时刻相同」——前者挡住 pid 被回收成别的程序，后者挡住「同 pid 上又起了个命令行一模一样的进程」（如用户在同一 jar 上另开了服务端）；任一项对不上即**拒绝杀**并清理该陈旧记录，绝不误伤用户自己起的服务端。
  - **收尾反馈说真话**（`task/ProcessStopper.kt` + `bot/ProcessReaper.kt`）：收尾器不再返回 `Unit` 而是区分「已收尾 / pid 文件缺失 / 进程已退出 / 文件内容非法」，停任务据此打印收尾摘要（含台账兜底数）、对「本次收回了上一轮残留」明确提示「前一次构建应是被强制中断」，并在**收尾后复验端口**——仍被占用时告警并给出手工清理命令。收尾仍不因此判失败：端口可能是用户自己起的服务端，任务无权替用户下结论。
...
- **`ExternalArtifactSource` + `ExternalArtifactProvisioner`：声明式外部制品源（ADR-0017）**：内置下载只覆盖六个平台，而验收常需注入平台枚举之外的第三方插件——此前只能各自手写「查 API → 解析地址 → 下载 → 缓存」，且没有 sha256 校验与来源溯源。现支持两种形态：`FixedUrl`（地址已知且稳定，如 Hangar 的固定版本下载端点）与 `ApiResolved`（地址须先查 API，如返回 URL 内嵌内容哈希、随再上传而变）；配套 `ArtifactUrlResolver`（响应文本 → 下载地址的纯函数，便利实现 `lastUrlMatch` 取末项，对应「版本数组按升序、末项即最新」的常见约定）。缓存落 `<cacheRoot>/external/<id>/<fileName>`（与 `<platform>/`、`maven/` 并列，运维可辨识、可清理），**命中缓存不发网络**——这条对 API 型来源尤其关键，否则离线 / 弱网复验会被外网波动阻塞；落盘走同目录临时文件 + `ATOMIC_MOVE`，并发读者只见「无文件」或「完整文件」。**刻意不做**（守 ADR-0001「保持精简」）：不实现市场 API / 搜索 / 版本列表 / 鉴权管理，不做依赖传递解析。可序列化是硬约束（消费方会捕获进任务动作，配置缓存拒不可序列化者），故 `lastUrlMatch` 用具名类而非 lambda 实现——Kotlin SAM 转换产出的 lambda 不是 `Serializable`，用它会静默埋雷：自己工程内单测正常，一旦被消费方捕获进 `BuildService` 参数或任务动作，配置缓存存储即失败（消费方多启用严格模式 `problems=fail`，表现为构建直接失败且归因困难）。

### 变更
- **内置下载支持 HTTP 代理**：此前 `Downloader` 用裸 `URL.openConnection()`，**完全不读代理配置**——用户在 `gradle.properties` 里配的 `systemProp.http.proxyHost` 对下载毫无作用，弱网 / 需代理的环境下只能干等超时。现自动读取 `http.proxyHost` / `https.proxyHost`（按目标地址 scheme 分组，即 Gradle `systemProp.*` 施加到守护进程的系统属性），未配置该组时回退环境变量 `HTTP_PROXY` / `HTTPS_PROXY`（两种大小写都认——JVM 原生不读它，而容器 / CI 常只提供这个）。两份绕过清单按**各自真实语义**判定（两者不同，曾因此写错）：`http.nonProxyHosts` 依 Java 语义（`|` 分隔，子域须显式写 `*.`），`NO_PROXY` 依 curl 语义（`,` 分隔，**裸域名同时命中其子域**、支持 `*`、容忍前导点与可选 `:port`）。未配置任何代理时行为与引入前完全一致（直连）。**不接管代理认证**——带 userinfo 的地址只取 host:port，需要认证请走系统属性形式交由 JVM 的 `Authenticator`。
- **内置下载回报进度**：`Downloader.download` 新增第四参 `progress: DownloadProgress`（默认不回报，加法、非破坏）。此前十几 MB 的服务端 jar 下载期间**完全无输出**，用户无法判断是在下载还是卡死——原实现刻意不耦合 Gradle 的 `ProgressLogger`（内部 API），代价就是没有任何反馈。现以不依赖 Gradle 的回调补齐，并附便于直接使用的节流实现 `DownloadProgress.logging(logger, label, percentStep)`（64 KiB 一块意味着 15 MB 要回调约 240 次，不节流会刷屏；总长未知时退化为按 MB 节流）。**框架自身的下载路径默认已开启该日志**（`ServerJarProvisioner` 起服任务、Waterfall 模块预置、外部制品源），消费方无需接线。`DownloadProgress.logging` 刻意**不实现 `Serializable`**——它持有日志 lambda，而 Kotlin lambda 不可序列化，声称可序列化会埋下「配置缓存存储期才失败」的雷（同 `ArtifactUrlResolver` 记录的坑）；其用法天然不跨序列化边界（下载发生在任务执行期，对象随用随弃）。
