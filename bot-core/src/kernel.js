...
// mc-testkit E2E 机器人公共内核（共享胶水构件 共享机器人包 @wcpe/mc-testkit-bot）。
//
// 职责（通用内核，不含任何业务场景）：
// 1. 读 MC_TESTKIT_E2E_* 契约环境变量（docs/API.md §3.3）组装连接 / 超时 / 压测配置；
// 2. 裸 TCP 探测端口 → mineflayer 登录 → spawn 后按 action 从注入的 scenarios 表取场景驱动；
// 3. 连接失败按重试间隔重连直到总超时；崩溃接管场景支持「断线后经代理重连到存活后端」；
// 4. 优雅收尾（SIGINT / SIGTERM），不残留连接。
//
// 场景驱动函数签名：async ({ bot, config, log, reconnectOnDisconnect }) => {}
// 消费方把自己的 action → 驱动函数表传进来，业务场景留在消费方项目。
...
// 崩溃接管：当前后端宕机断线后，等这么久再经代理重连——给宕机后端端口关闭 + 代理感知留时间，
// 避免重连又落回正在退出的后端
const RECONNECT_AFTER_CRASH_DELAY_MS = 3000

/**
 * 启动一个 mc-testkit E2E 机器人。
 *
 * @param {Object} options
 * @param {Object<string, Function>} options.scenarios action → 场景驱动函数（见文件头注释的签名）。
 */
function runBot({ scenarios }) {
  const config = {
    ...
    // ── 压测维度（压测编排；仅 continuous-stress 场景用）──
  ...
  // 崩溃接管：场景登记期望重连后，下面两个变量驱动「断线 → 经代理 fallback 重连到存活后端」
  ...
  // action → 场景驱动分发表：消费方经 scenarios 注入自己的 action → 驱动函数。
  function scenarioRunner() {
    const runner = scenarios && scenarios[config.action]
  ...
  // 场景登记「断线后重连」（崩溃接管）：设置期望并返回一个在「重连后再次 spawn」时 resolve（带新 bot）
