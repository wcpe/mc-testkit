...
- **编排插件**（`src/main/kotlin/top/wcpe/mc/testkit/`，本仓库核心；纯 Kotlin 实现、KTS 构建）
  ...
  - **内部包接缝**（让第一期各 FR 各占一包、文件隔离并行；入口与共享契约只在 FR-01 落地一次）：
    ...
    - `dsl/`（FR-01 冻结形态 → FR-22 加法扩展）：平台枚举（Paper/Folia/Spigot + 三代理，见 ADR-0003/0013）+ DslMarker、Backend/Proxy/Scenario/Bot/Dependencies Spec；proxy 可声明独立版本与 Java 主版本，backend/proxy 可声明 JVM 参数与 javaagent。
