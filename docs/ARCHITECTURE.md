...
- **编排插件**（`src/main/kotlin/top/wcpe/mc/testkit/`，本仓库核心；纯 Kotlin 实现、KTS 构建）
  ...
  - `McTestkitExtension`（DSL 扩展）：声明拓扑（代理/后端/路由）、场景、依赖注入项，以及 backend/proxy 的节点环境、模板与代理专属插件；只忠实记录声明，不含行为。
  - **内部包接缝**（让第一期各 FR 各占一包、文件隔离并行；入口与共享契约只在 FR-01 落地一次）：
    ...
    - `model/`（FR-03）：拓扑模型（`Topology`/`ResolvedBackend`/`ResolvedProxy`）与纯函数解析、端口推导（基数+序号）、配置期校验（节点名唯一且后端/代理不相撞、路由目标存在、端口不冲突、场景引用存在）。解析模型同时携带 FR-20 的节点环境、模板声明与代理专属插件声明，并在配置期拦截非法环境变量名及大小写不敏感的 `MC_TESTKIT_E2E_` 保留前缀。任务图无环属 `task/`（FR-04）。
