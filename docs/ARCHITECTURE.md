...
- **编排插件**（`src/main/kotlin/top/wcpe/mc/testkit/`，本仓库核心；纯 Kotlin 实现、KTS 构建）
  ...
  - **内部包接缝**（让第一期各 FR 各占一包、文件隔离并行；入口与共享契约只在 FR-01 落地一次）：
    - `contract/`（FR-01 冻结，全包共享）：对外契约常量——插件 id/扩展名/默认值、环境变量名集（前缀 `MC_TESTKIT_E2E_`）、任务命名约定、机器人↔桩控制协议、结果文件约定、运行目录布局（`McTestkitRunDirectories`：工作根 / `run-<后端名>` / `run-proxy`，与任务侧路径推导同源，消费方据此注入配置与验收桩）。
    - `dsl/`（FR-01 冻结形态 → FR-22 加法扩展）：平台枚举（Paper/Folia/Spigot + 三代理，见 ADR-0003/0013）+ DslMarker、Backend/Proxy/Scenario/Bot/Dependencies Spec；proxy 可声明独立版本与 Java 主版本，backend/proxy 可声明 JVM 参数与 javaagent。扩展另暴露只读访问器 `backendRunDirectory(name)`，返回该后端运行目录的 `Provider<Directory>`，供消费方注入配置 / 验收桩（见 docs/API.md §3.1）。
    ...
    - `provision/`（FR-02/FR-22/FR-23）：内置服务端/代理下载与运行——Paper/Folia/Velocity/Waterfall 经 PaperMC 下载 API、BungeeCord 经 SpigotMC Jenkins、Spigot 经受控公共构件源（首源 GetBukkit + 镜像回退，下载后写 `source.properties` 记录来源/版本/本地 SHA-256，见 ADR-0013），带本地缓存与校验；按节点平台+版本解析 jar，并由 `JavaRuntimeSelector` 选择节点 JVM。`ServerLauncher` 按构件形态选路：自包含 jar 与 paperclip 引导件（按包前缀 `io.papermc.paperclip.` 识别，覆盖两个实测入口名）走 `java -jar`，运行目录带注入运行库目录 `server-libraries/` 的 thin jar 改为经只含清单的启动器 jar 传完整 classpath（`Class-Path` 条目按 UTF-8 百分号编码，规避空格 / 中文被截断）；paperclip 自有的 `libraries/` 不参与 classpath，避免其跨轮保留的下载物污染下一轮启动。**自实现、保持精简**，不搬运插件市场下载等非必需能力。
      - **公开面与外部制品源（FR-23，ADR-0017）**：`PaperDownloadsApi` / `PaperDownload`、`Downloader`、`WaterfallModuleProvisioner`、`File.sha256()` 连同已公开的 `ServerJarProvisioner` / `ServerLauncher` / `JavaRuntimeSelector` 构成**对外可复用的下载/运行基建**，使消费方可只复用底层、自建编排（如驱动真实游戏客户端的验收编排），不必接受整体编排模型。仍留内部：`ProvisionPlatform`（DSL 侧有 `dsl/Platforms` 作对外平台枚举，本枚举只供内部解析下载源）、`JarProvisionService` / `JarCache`（签名依赖 `ProvisionPlatform`，对外能力已由 `ServerJarProvisioner` 以 `String` 平台完整覆盖）。`ExternalArtifactProvisioner` + `ExternalArtifactSource`（`FixedUrl` / `ApiResolved`）+ `ArtifactUrlResolver` 提供**平台枚举之外**的第三方制品来源，缓存落 `<cacheRoot>/external/<id>/<fileName>`（与 `<platform>/`、`maven/` 并列）。**边界**：公开面即契约（签名变更按 SemVer 升 major），公开类型须可序列化（消费方会捕获进任务动作，配置缓存拒不可序列化者）；外部源刻意不做市场客户端形态（市场 API / 搜索 / 版本列表 / 鉴权），不做依赖传递解析——新增源类型须先证明声明式表达不了。
