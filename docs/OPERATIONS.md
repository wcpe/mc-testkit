...
- **服务端起不来且报 `ClassNotFoundException`**：先看运行目录的 `<key>.log`，再看启动日志里的选路行（`按自包含 jar 启动` / `按运行库 classpath 启动`）。运行目录带 `libraries/` 的构件走后者，依赖由 `.mc-testkit-<key>-classpath.jar` 的清单给出；该清单是**启动时刻**的快照，运行库有变动需重跑任务重建。
