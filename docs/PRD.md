...
| FR-02 | 内置下载与运行：插件自实现下载并运行 Paper/Folia/Spigot 后端与 Velocity/Waterfall/BungeeCord 代理（Spigot 走受控公共构件源 + 多源回退 + 溯源，见 ADR-0013）；启动按构件形态选路（自包含 jar / paperclip 引导件走 `java -jar`，运行目录带注入运行库目录 `server-libraries/` 的 thin jar 经启动器 jar 传 classpath）（内置自实现，不外挂第三方下载库，见 ADR-0001）| P1 | 已交付@v0.1.0 |
...
| FR-23 | 开放下载/运行基建为公开 API + 声明式外部制品源：把 `provision/` 中稳定的下载与运行能力（`PaperDownloadsApi` / `Downloader` / `WaterfallModuleProvisioner` / `File.sha256()`，连同已公开的 `ServerJarProvisioner` / `ServerLauncher` / `JavaRuntimeSelector`）开放给消费方，使其可**只复用底层、自建编排**而不必接受整体编排模型；并提供声明式的平台枚举之外制品来源（固定 URL / API 动态解析），覆盖第三方插件注入场景。两种形态均为加法变更，既有 DSL / 任务名 / env / 协议不变（见 [ADR-0017](adr/0017-provision-api-and-external-artifact-sources.md)、docs/specs/fr-23-provision-api-and-external-artifacts.md）| P2 | 已交付@v0.12.0 |
| FR-24 | 场景生命周期钩子：为 `scenario` 增加 `beforeScenario` / `readyScenario` / `afterScenario` 三个时序点的钩子，供消费方插入「起外部依赖进程 → 等它就绪 → 依赖服务端的初始化（审批 / 造数）→ 收尾」这类时序敏感动作，而无需自建整套编排；配套 `ExecHook` / `StopPidHook` / `HttpHook` / `SleepHook` / `HookChain` 五个内置实现（均可序列化，满足配置缓存）。收尾钩子挂 `finalizedBy`，正常 / 失败 / 中断三路径都执行；`readyScenario` 仅集群场景提供，其他形态配置期报错不静默忽略。属加法变更（见 [ADR-0020](adr/0020-scenario-lifecycle-hooks.md)）| P1 | 已交付@v0.14.0 |
...
- [x] FR-02 thin jar 启动：运行目录带注入运行库目录 `server-libraries/` 的构件按 `-cp <启动器 jar> <Main-Class>` 拉起，`Class-Path` 条目按 UTF-8 百分号编码（含空格 / 中文路径不被截断）；自包含 jar、paperclip 主入口（含 1.8.8–1.17.1 的 `io.papermc.paperclip.Paperclip`）、读不出 `Main-Class` 三种情况退回 `-jar`；paperclip 自有的 `libraries/` 不参与 classpath；既有启动路径与全部单测回归通过（**单测维度**；thin jar 平台的真实拉起待下次实机维度一并确认）。
