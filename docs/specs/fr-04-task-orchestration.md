...
- **进程收尾的极端残留**：异常强杀仍可能在极端情况留残留（ADR-0004 已记），排障清理指引归运维文档；本 FR 用 `finalizedBy` + `try/finally` 双保险尽量兜住正常/失败/中断三路径。**后续加固**：pid 文件是「当前一轮」的凭证（起服时会被覆盖、随运行目录清理消失），故仅靠它收尾不掉上一轮被强杀留下的逃逸进程；现新增**进程台账**（落结果目录，见 `task/ProcessLedger`）作跨轮次兜底，停任务先按 pid 文件、再按台账收尾（杀前核对命令行与启动时刻是否都一致，pid 复用则拒杀），并在起服前做**端口占用预检**（被占即中文失败并给出按端口排查命令），消除「就绪门被残留进程误判为已就绪 → 新进程 bind 失败」的误导形态。
