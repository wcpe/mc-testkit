...
- **纯函数展开器**（`task/BotProcessPlanner.kt`，新增，可穷举单测）：把场景 `botSpecs` 展开为每进程一项的 `BotProcessPlan(action, username, key, botIndex?, env)`——
...
- **编排**（`task/McTestkitTasks.kt`）：
  ...
  - 新增 `launchScenarioBots(...)`：按 `BotProcessPlanner.expand` 逐 plan 起进程，进程数 >1 时**强制** `BOT_USERNAME=plan.username`（末位合入，盖过消费方单值 override，保证唯一），下发 `BOT_INDEX`（同质）与 `sharedExtraEnv`（集群的 `CLUSTER_BACKENDS`）；返回全部 `Process`。
...
- [x] 纯函数：`BotProcessPlanner.expand` + 穷举单测（单 bot 向后兼容 / 同质 count=N / 异质双角色 / env 透传）。
- [x] 配置期校验：count≥1、多 bot 唯一 role、压测禁 count/多 bot + 单测。
- [x] 编排：`launchBotProcess` 加 key、`launchScenarioBots`；单后端 / 经代理 / 集群三路径起多 bot 并全收尾。
- [x] 功能测试（TestKit）：集群 `count=N` 与单后端双角色注册任务 + 配置期中文报错。
- [x] 文档同步：PRD（FR-16）、ARCHITECTURE（多 bot 机制）、API（DSL + 任务表注）、CHANGELOG、ADR-0009。
- [x] 单元/TestKit + 根 `build` 绿（含 ktlint），向后兼容既有测试不改语义（177 测试全绿、新增 27）。
