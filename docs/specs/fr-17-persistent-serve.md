...
> **控制台命令（已交付，2026-09-26 增补）**：本节原先把「stdin 转发」列为范围外，但 `docs/API.md` §5 已承诺 `ServerLauncher.launch` 返回的 `Process`「stdin 保持打开，可向控制台注入命令」——文档承诺与实现不一致，且 serve 挂住后**无法当场停服**，只能另开终端或 Ctrl+C。现已补齐：`serve<Key>` 挂住期间终端输入逐行下发到子进程控制台（直连 → 后端；经代理 / 集群 → 代理），结束路径安静（EOF / 目标退出 / 中断均不报错，仅「目标存活却写不进」才告警）。**只 serve 接线**：`e2e*` 自动化任务不接收终端输入，保持结果确定性。详见 `docs/API.md` §3.2.1。范围内的其余条款不变。
