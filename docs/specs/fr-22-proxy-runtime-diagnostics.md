...
- `ServerLauncher` 继续形成 `java <jvmArgs> <-javaagent> -jar ...`，对不存在/非文件 agent 给出中文错误；所有启动入口复用同一个参数构造器，禁止各任务复制拼接。
...
- [ ] 先补 DSL 默认值、解析、Java 主版本选择、agent 路径、参数顺序与失败提示测试。
- [ ] 扩展 DSL/Resolved 模型并统一所有启动路径。
- [ ] 增加 Velocity 3.1.1、最新 3.x（固定 3.5.1）、4.1.0 下载与 Java 运行矩阵。
- [ ] 增加消费方 fixture 进程接线示例和结果文件门禁测试。
- [ ] 发布 `0.7.0-SNAPSHOT` 到 Maven Local，ServerProbe 按插件版本消费并完成真实矩阵。
- [ ] ServerProbe 验收通过后再走 mc-testkit 正式版本发布。
- [ ] 文档同步：PRD 状态、ARCHITECTURE、DSL、README、CHANGELOG。
